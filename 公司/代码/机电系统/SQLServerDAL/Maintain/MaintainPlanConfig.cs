...
{
    public class MaintainPlanConfig:IMaintainPlanConfig
    {
        public QueryParam GenerateSearchTerm(MaintainPlanConfigInfo item)
        {
            ...
            if (item.PlanType == MaintainPlanType.DailyPatrol || item.PlanType == MaintainPlanType.RoutineInspect || item.PlanType == MaintainPlanType.RoutineMaintain)
                sqlSearch += " and PlanType =" + (int)item.PlanType;

            if (item.PlanObject != null && item.PlanObject != "")
                sqlSearch += " and PlanObject like '%" + item.PlanObject + "%'";
            QueryParam searchTerm = new QueryParam();
            searchTerm.TableName = "FM2E_MaintainPlanConfig a left join FM2E_System b on a.System = b.SystemID left join FM2E_SubSystem c on a.Subsystem = c.SubSystemID left join FM2E_Company g on a.CompanyID = g.CompanyID";
        ...
        public void InsertMaintainPlanConfig(MaintainPlanConfigInfo model)
        {
            ...
            strSql.Append("insert into FM2E_MaintainPlanConfig(");
            strSql.Append("PlanPeriod,PeriodUnit,System,Subsystem,PlanObject,PlanContent,CheckStandard,CompanyID,PlanType)");
            strSql.Append(" values (");
            strSql.Append("@PlanPeriod,@PeriodUnit,@System,@Subsystem,@PlanObject,@PlanContent,@CheckStandard,@CompanyID,@PlanType)");
            strSql.Append(";select @@IDENTITY");
            SqlParameter[] parameters = {
					new SqlParameter("@PlanPeriod", SqlDbType.Int,4),
					...
					new SqlParameter("@PlanObject", SqlDbType.NVarChar,50),
					new SqlParameter("@PlanContent", SqlDbType.NVarChar,200),
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update FM2E_MaintainPlanConfig set ");
            strSql.Append("PlanPeriod=@PlanPeriod,");
            strSql.Append("PeriodUnit=@PeriodUnit,");
            strSql.Append("System=@System,");
            strSql.Append("Subsystem=@Subsystem,");
            strSql.Append("PlanObject=@PlanObject,");
        ...
        /// 删除一条数据
        /// </summary>
        public void DelMaintainPlanConfig(long ItemID)
        {
            try
            {
                StringBuilder strSql = new StringBuilder();
                strSql.Append("delete FM2E_MaintainPlanConfig ");
                strSql.Append(" where ItemID=@ItemID ");
                SqlParameter[] parameters = {
        ...
        /// 得到一个对象实体
        /// </summary>
        public MaintainPlanConfigInfo GetMaintainPlanConfig(long ItemID)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 a.*,b.SystemName as SystemName,c.SubSystemName as SubSystemName,g.CompanyName from FM2E_MaintainPlanConfig  a left join FM2E_System b on a.System = b.SystemID left join FM2E_SubSystem c on a.Subsystem = c.SubSystemID left join FM2E_Company g on a.CompanyID = g.CompanyID");
            strSql.Append(" where ItemID=@ItemID ");
            SqlParameter[] parameters = {
					new SqlParameter("@ItemID", SqlDbType.BigInt)};
            parameters[0].Value = ItemID;
        ...
        private MaintainPlanConfigInfo GetData(IDataReader rd)
        {
            MaintainPlanConfigInfo item = new MaintainPlanConfigInfo();
            ...
            if (!Convert.IsDBNull(rd["PlanPeriod"]))
                item.PlanPeriod = Convert.ToInt32(rd["PlanPeriod"]);

            if (!Convert.IsDBNull(rd["PeriodUnit"]))
                item.PeriodUnit = (MaintainPlanPeriodUnit)Convert.ToInt32(rd["PeriodUnit"]);
            ...
            if (!Convert.IsDBNull(rd["PlanObject"]))
                item.PlanObject = Convert.ToString(rd["PlanObject"]);

            if (!Convert.IsDBNull(rd["PlanContent"]))
                item.PlanContent = Convert.ToString(rd["PlanContent"]);
        ...
        public IList GetAllList(MaintainPlanConfigInfo item)
        {
            string sqlSearch = "where 1=1";
            if (item.Subsystem != 0)
                sqlSearch += " and Subsystem ='" + item.Subsystem + "'";
            ...
            string sql = string.Format("select a.*,b.SystemName as SystemName,c.SubSystemName as SubSystemName,g.CompanyName from FM2E_MaintainPlanConfig a left join FM2E_System b on a.System = b.SystemID left join FM2E_SubSystem c on a.Subsystem = c.SubSystemID left join FM2E_Company g on a.CompanyID = g.CompanyID {0}", sqlSearch);
            ...
            try
            {
                using (SqlDataReader rd = SQLHelper.ExecuteReader(SQLHelper.ConnectionString, CommandType.Text, sql, null))
                {
                    while (rd.Read())
                    {
                        MaintainPlanConfigInfo i = this.GetData(rd);
                        list.Add(i);
                    }
                }
            }
        ...
        {
            string sql = string.Format("select EquipmentNO from FM2E_MaintainPlanEquipment where TableName='FM2E_MaintainPlanConfig' and ItemID='{0}'", itemID);
            ArrayList list = new ArrayList();
            string item = "";
            try
            {
        ...
        public void UpdateEquipments(MaintainPlanConfigInfo model)
        {
            ...
            try
            {
                ...
                delSql.Append(" where ItemID=@ItemID and TableName='FM2E_MaintainPlanConfig'");
                ...
                if (model.EquipmentList != null)
                {
                    ...
                    parameters[0].Value = "FM2E_MaintainPlanConfig";
        ...
        {
            string sql = string.Format("select a.*,b.SystemName as SystemName,c.SubSystemName as SubSystemName,g.CompanyName from FM2E_MaintainPlanEquipment s1 left join FM2E_MaintainPlanConfig a on a.ItemID = s1.ItemID left join FM2E_System b on a.System = b.SystemID left join FM2E_SubSystem c on a.Subsystem = c.SubSystemID left join FM2E_Company g on a.CompanyID = g.CompanyID where s1.tablename='FM2E_MaintainPlanConfig' and s1.EquipmentNO ='{0}' and a.PlanType='{1}' ", EquipmentNO,(int)PlanType);
            ArrayList list = new ArrayList();
            try
            {
                using (SqlDataReader rd = SQLHelper.ExecuteReader(SQLHelper.ConnectionString, CommandType.Text, sql, null))
