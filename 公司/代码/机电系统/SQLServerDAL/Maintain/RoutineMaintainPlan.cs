...
{
    public class RoutineMaintainPlan : IRoutineMaintainPlan
    {
        public QueryParam GenerateSearchTerm(RoutineMaintainPlanInfo item, string[] WFStates)
        {
            ...
            {
                sqlSearch += "and h.TableName='FM2E_RoutineMaintainPlan' and (";
                bool first = true;
            ...
            searchTerm.TableName = "FM2E_RoutineMaintainPlan a left join FM2E_User b on a.Planner=b.UserName left join FM2E_User c on a.Approvaler=c.UserName left join FM2E_Company g on a.CompanyID = g.CompanyID left join FM2E_WorkflowInstance h on a.PlanID=h.DataID left join FM2E_Department s on s.DepartmentID=a.DepartmentID";
        ...
        public QueryParam GenerateSearchTerm(RoutineMaintainPlanInfo item)
        {
            ...
            QueryParam searchTerm = new QueryParam();
            searchTerm.TableName = "FM2E_RoutineMaintainPlan a left join FM2E_User b on a.Planner=b.UserName left join FM2E_User c on a.Approvaler=c.UserName left join FM2E_Company g on a.CompanyID = g.CompanyID left join FM2E_Department s on s.DepartmentID=a.DepartmentID ";
            searchTerm.ReturnFields = "a.*,b.PersonName as PlannerName,c.PersonName as ApprovalerName,g.CompanyName as CompanyName,s.Name as DepartmentName";
            searchTerm.PageSize = 10;
            searchTerm.OrderBy = "order by PlanDate desc";
            searchTerm.Where = sqlSearch;
        ...
        public long InsertRoutineMaintainPlan(RoutineMaintainPlanInfo model)
        {
            ...
            try
            {
                ...
                {
                    foreach (RoutineMaintainDetailInfo item in model.PlanDetailList)
                    {
                        item.PlanID = thisID;
                        InsertRoutineMaintainDetail(trans, item);
                    }
                }
                //事务提交
                trans.Commit();
        ...
        private long insertPlan(SqlTransaction trans, RoutineMaintainPlanInfo model)
        {
            long id = 1;
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into FM2E_RoutineMaintainPlan(");
            ...
            {
                throw new DALException("添加例行保养计划信息失败", e);
            }
            finally
            {
                rdr.Close();
        ...
        /// 增加一条数据
        /// </summary>
        private void InsertRoutineMaintainDetail(SqlTransaction trans, RoutineMaintainDetailInfo model)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        public void UpdateRoutineMaintainPlan(RoutineMaintainPlanInfo model)
        {
            ...
            try
            {
                ...
                delSql.AppendFormat("delete FM2E_RoutineMaintainDetail");
        ...
        /// 更新计划信息
        /// </summary>
        private void updatePlan(SqlTransaction trans, RoutineMaintainPlanInfo model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update FM2E_RoutineMaintainPlan set ");
            ...
            {
                throw new DALException("更新例行保养计划信息失败", e);
            }
        }
        ...
        /// 删除一条数据
        /// </summary>
        public void DelRoutineMaintainPlan(long PlanID)
        {
            SqlConnection conn = null;
            SqlTransaction trans = null;
            try
            {
                ...
                //删除申请明细信息
                DelRoutineMaintainDetail(trans, PlanID);
        ...
        private void DelPlan(SqlTransaction trans, long PlanID)
        {
            ...
            {
                StringBuilder strSql = new StringBuilder();
                strSql.Append("delete FM2E_RoutineMaintainPlan ");
                strSql.Append(" where PlanID=@PlanID ");
                SqlParameter[] parameters = {
					new SqlParameter("@PlanID", SqlDbType.BigInt)};
            ...
            {
                throw new DALException("删除例行保养计划信息失败", e);
            }
        }
        /// <summary>
        /// 删除所有申请明细
        /// </summary>
        private void DelRoutineMaintainDetail(SqlTransaction trans, long PlanID)
        {
            try
            {
                StringBuilder strSql = new StringBuilder();
        ...
        /// 得到一个对象实体
        /// </summary>
        public RoutineMaintainPlanInfo GetRoutineMaintainPlan(long PlanID)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 a.*,b.PersonName as PlannerName,c.PersonName as ApprovalerName,g.CompanyName as CompanyName,s.Name as DepartmentName from FM2E_RoutineMaintainPlan  a left join FM2E_User b on a.Planner=b.UserName left join FM2E_User c on a.Approvaler=c.UserName left join FM2E_Department s on s.DepartmentID=a.DepartmentID left join FM2E_Company g on a.CompanyID = g.CompanyID");
            strSql.Append(" where a.PlanID=@PlanID ");
            SqlParameter[] parameters = {
					new SqlParameter("@PlanID", SqlDbType.BigInt)};
            parameters[0].Value = PlanID;
            RoutineMaintainPlanInfo model = null;
            try
            {
                ...
                strDetailSql.Append("select a.*,b.SystemName as SystemName,c.SubSystemName as SubSystemName from FM2E_RoutineMaintainDetail a left join FM2E_System b on a.System = b.SystemID left join FM2E_SubSystem c on a.Subsystem = c.SubSystemID ");
                ...
                using (SqlDataReader rd = SQLHelper.ExecuteReader(SQLHelper.ConnectionString, CommandType.Text, strDetailSql.ToString(), parameters))
                {
                    while (rd.Read())
                    {
                        RoutineMaintainDetailInfo item = GetDetailData(rd);
                        list.Add(item);
                    }
                }
            ...
            {
                throw new DALException("获取例行保养计划信息失败", e);
            }
            return model;
        }

        private RoutineMaintainPlanInfo GetData(IDataReader rd)
        {
            ...
            if (!Convert.IsDBNull(rd["Status"]))
                item.Status = (RoutineMaintainPlanStatus)(Convert.ToInt32(rd["Status"]));
        ...
        private RoutineMaintainDetailInfo GetDetailData(IDataReader rd)
        {
