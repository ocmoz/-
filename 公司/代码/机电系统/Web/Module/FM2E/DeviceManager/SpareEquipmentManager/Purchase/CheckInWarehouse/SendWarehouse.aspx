<%@ Page Title="送仓报验" Language="C#" MasterPageFile="~/MasterPage/MasterPage.master" EnableEventValidation="false"
    AutoEventWireup="true" CodeFile="SendWarehouse.aspx.cs" Inherits="Module_FM2E_DeviceManager_SpareEquipmentManager_Purchase_CheckInWarehouse_SendWarehouse" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="PageBody" runat="Server">
    ...
    <script type="text/javascript">
        ...
        function setModalPopup(button_id, isedit) {
            ...
            if (isedit) {
                ...
                //采购人
                var s = document.getElementById('<%= DropDownList_Purchaser.ClientID %>');
                var whid = document.getElementById(button_id.replace('ImageButton_Edit', 'Hidden_Purchaser')).value;
                ...
                //验收数量
                var ac_count = document.getElementById(button_id.replace('ImageButton_Edit', 'Label_AcceptanceCount')).innerText.replace(regS, "");
                document.getElementById('<%= TextBox_AcceptedCount.ClientID %>').value = ac_count;
                //生产商
                ...
                //验收备注
                var remrk = document.getElementById(button_id.replace('ImageButton_Edit', 'Label_AcceptanceRemark')).innerText;
                document.getElementById('<%= TextBox_RemarkAccept.ClientID %>').value = remrk;
                //金额小计
                document.getElementById('<%= TextBox_Amount.ClientID %>').value = parseFloat(unitprice) * parseFloat(count);
                //技术人员ID
                var ct = document.getElementById(button_id.replace('ImageButton_Edit', 'Label_Checker_Technician')).innerText;
                document.getElementById('<%= TextBox_TechnicianID.ClientID %>').value = ct;
                //清空技术人员密码
                document.getElementById('<%= TextBox_TechnicianPassword.ClientID %>').value = '';
                //系统
            ...
            //新增
                ...
                //验收备注
                document.getElementById('<%= TextBox_RemarkAccept.ClientID %>').value = "";
                //金额小计
        ...
        function saveEditItem() {
            ...
            //采购人密码
            var p = trim(document.getElementById('<%= TextBox_PurchaserPassword.ClientID %>').value);
            if (p.length == 0) {
                //alert('请输入采购人密码');
                //return false;
            }
            //技术验收人
            var p = trim(document.getElementById('<%= TextBox_TechnicianID.ClientID %>').value);
            if (p.length == 0) {
                alert('请输入技术验收人用户名');
                return false;
            }
            //技术验收人密码
            var p = trim(document.getElementById('<%= TextBox_TechnicianPassword.ClientID %>').value);
            if (p.length == 0) {
                alert('请输入技术验收人密码');
                return false;
            }
    ...
    <asp:Panel ID="Panel_EditItem" runat="server" Style="width: 95%; height: 400px; display: none"
        CssClass="modalPopup">
        <table style="width: 100%; border-collapse: collapse; vertical-align: middle; text-align: left;
            ...
            <tr>
                ...
                <td class="table_none table_none_NoWidth" style="height: 30px;" colspan="3">
                    <asp:DropDownList ID="DropDownList_Purchaser" DataTextField="UserIDPurchaserNameRemark"
                        DataValueField="UserID" runat="server">
                    </asp:DropDownList>
                    <span style="color: Red; font-weight: bold">*</span> <span style="display: none">密码：<asp:TextBox
                        ID="TextBox_PurchaserPassword" runat="server" TextMode="Password"></asp:TextBox><span
            ...
            <tr>
                ...
                <td class="table_none table_none_NoWidth" colspan="3" style="height: 30px;">
                    <asp:TextBox ID="TextBox_RemarkAccept" runat="server" title="请输入备注~50:" Width="300px"></asp:TextBox>
        ...
        <table id="RootTable" style="width: 100%; border-collapse: collapse; vertical-align: middle;
            ...
            <tr>
                ...
                <td style="text-align: center" colspan="2">
                    表单编号:<asp:Label ID="Label_SheetID" runat="server" Font-Underline="true"></asp:Label>
                </td>
            </tr>
            ...
            <tr>
                <td style="text-align: center" colspan="2">
                     送仓仓库：<asp:DropDownList ID="ddl_Warehouse"  runat="server">
                    </asp:DropDownList>   
                </td>               
            </tr>
            <tr>
                                ...
                                <ItemTemplate>
                                    <tr id="tr_item" runat="server">
                                        ...
                                        <td>
                                            ...
                                            <input type="hidden" runat="server" value='<%# Eval("Purchaser") %>' id="Hidden_Purchaser" />
            ...
            <tr>
                <td colspan="4" style="text-align: right">
                    <asp:Button ID="Button_Submit" runat="server" Text="验收" CssClass="button_bak" OnClick="Button_Submit_Click"
                        OnClientClick="javascript:return confirm('确认验收？');" />
                </td>
            </tr>
        </table>
    </div>
