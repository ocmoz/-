...
public partial class Module_FM2E_DeviceManager_SpareEquipmentManager_Purchase_ExecutePurchasing_ConfirmCAForm : System.Web.UI.Page
{
    ...
    string sessionName = "Module_FM2E_DeviceManager_SpareEquipmentManager_Purchase_ExecutePurchasing_ConfirmCAForm";
    ...
    protected void Repeater_ItemList_RowDataBound(object sender, RepeaterItemEventArgs e)
    {
        ...
        //if (!item.PurchaserConfirm && item.Purchaser == Common.Get_UserName)
        if (!item.PurchaserConfirm)
        {
            tr.BgColor = "LightYellow";

            e.Item.FindControl("Button_Confirm").Visible = true;
        }
        else
        {
            tr.BgColor = "Transparent";
            e.Item.FindControl("Button_Confirm").Visible = false;
        }
    ...
    protected void Repeater_ItemList_RowCommand(object sender, RepeaterCommandEventArgs e)
    {
        if (e.CommandName != null && e.CommandName != "")
        {
            ...
            //int tempid = Convert.ToInt32(e.Item.FindControl("Hidden_FormID").ToString());
            Label labelid = (Label)e.Item.FindControl("Label_ID");
            int tempid = Convert.ToInt32(labelid.Text.Trim());
            ...
            {
                if (item.ItemID == itemid && item.ID==tempid)
                {
                    target = item;
                    break;
                }
