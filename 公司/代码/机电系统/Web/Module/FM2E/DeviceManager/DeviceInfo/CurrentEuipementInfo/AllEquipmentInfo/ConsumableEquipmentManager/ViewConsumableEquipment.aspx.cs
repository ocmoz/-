...
public partial class Module_FM2E_DeviceManager_DeviceInfo_ConsumableEquipmentManager_ViewConsumableEquipment : System.Web.UI.Page
{
    ...
    private void FillData()
    {
        if (cmd == "view")
        {
            try
            {
                ...
                lbConsumableEquipmentID.Text = Convert.ToString(item.ConsumableEquipmentID);
                lbConsumableEquipmentNO.Text = item.ConsumableEquipmentNO;
                ViewState["EquipmentNO"] = item.ConsumableEquipmentNO;
                lbName.Text = item.Name;
                lbSystemID.Text = item.SystemID;
                lbSerialNum.Text = item.SerialNum;
                lbModel.Text = item.Model;
                lbSpecification.Text = item.Specification;
                lbAssertNumber.Text = item.AssertNumber;
                lbUnit.Text = item.Unit;
                lbCount.Text = Convert.ToString(item.Count);
                ...
                GridView2.DataSource = item.ConsumableEquipmentDetailList;
                GridView2.DataBind();
            }
            catch (Exception ex)
            {
        ...
        if (cmd == "delete")
        {
            ...
            if (bSuccess)
            {
                EventMessage.MessageBox(Msg_Type.Info, "操作成功", "删除" + id + "成功！", Icon_Type.OK, true, Common.GetHomeBaseUrl("ConsumableEquipmentList.aspx"), UrlType.Href, "");
            }
        }
    }
    //绑定按钮
    ...
    {
        HeadMenuWebControls1.ButtonList[0].ButtonUrl = "EditConsumableEquipment.aspx?cmd=edit&id=" + id;
