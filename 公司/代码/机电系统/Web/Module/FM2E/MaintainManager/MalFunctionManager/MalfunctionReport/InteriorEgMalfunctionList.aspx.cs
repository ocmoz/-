...
public partial class Module_FM2E_MaintainManager_MalFunctionManager_MalfunctionReport_InteriorEgMalfunctionList : System.Web.UI.Page
{
    ...
    private MalfunctionSearchInfo SearchTerm
    {
        get
        {
            if (ViewState["SearchTerm"] == null)
            {
                ...
                term.SystemID = UserData.CurrentUserData.IM;
                return term;
            }
    ...
    protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName == "check" && userdata1.CompanyID == "SG") 
        {
            ...
            {
                EventMessage.MessageBox(Msg_Type.Error, "操作失败", "故障处理单实际修复时间确认失败", ex, Icon_Type.Error, true, Common.GetHomeBaseUrl("InteriorEgMalfunctionList.aspx"), UrlType.JavaScript, "");
            }
            EventMessage.MessageBox(Msg_Type.Info, "操作成功", "故障处理单实际修复时间确认成功，故障处理单号为：" + model.SheetNO, Icon_Type.OK, true, Common.GetHomeBaseUrl("InteriorEgMalfunctionList.aspx"), UrlType.Href, "");
        }
        //********** Modification Finished 2012-4-3 **********************************************************************************************
        if (e.CommandName == "stationcheck" )
            //if (e.CommandName == "stationcheck" && b == true)
        {
            MalfunctionHandleInfo model = malfunctionBll.GetMalfunctionSheet(id);
            try
            {
                model.Status = MalfunctionHandleStatus.Waiting4Accept;
                model.Stationcheck = true;
                malfunctionBll.UpdateMalfunctionSheetBasicData(model);
            }
            catch (Exception ex)
            {
                EventMessage.MessageBox(Msg_Type.Error, "操作失败", "故障处理单确认失败", ex, Icon_Type.Error, true, Common.GetHomeBaseUrl("InteriorEgMalfunctionList.aspx"), UrlType.JavaScript, "");
            }
            EventMessage.MessageBox(Msg_Type.Info, "操作成功", "故障处理单确认成功，故障处理单号为：" + model.SheetNO, Icon_Type.OK, true, Common.GetHomeBaseUrl("InteriorEgMalfunctionList.aspx"), UrlType.Href, "");
        }
        //********** Modification Finished 2012-4-3 ********************************************************************************************** 
        else if (e.CommandName == "del")
        {
            ...
            EventMessage.MessageBox(Msg_Type.Info, "操作成功", "删除故障处理单成功！", Icon_Type.OK, true, Common.GetHomeBaseUrl("InteriorEgMalfunctionList.aspx"), UrlType.Href, "");
        }
    }
    /// <summary>
    /// GridView数据绑定事件
