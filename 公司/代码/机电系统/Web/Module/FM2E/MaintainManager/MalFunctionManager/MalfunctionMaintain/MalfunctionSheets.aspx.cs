...
public partial class Module_FM2E_MaintainManager_MalFunctionManager_MalfunctionMaintain_MalfunctionSheets : System.Web.UI.Page
{
    ...
    private void InitialPage()
    {
        try
        {
            ...
            ddlFilterMaintainTeam2.Items.Clear();
            ddlFilterMaintainTeam2.Items.Add(new ListItem("不限", "0"));
            ddlFilterMaintainTeam2.Items.AddRange(ListItemHelper.GetAllMaintainTeams(loginUser.CompanyID));
            ddlFilterMaintainTeam2.SelectedValue = loginUser.DepartmentID.ToString();
    ...
    private void PermissionControl()
    {
        ...
        //{
        //    GridView1.Rows[i].FindControl("ImageButton1").Visible = SystemPermission.CheckPermission(PopedomType.Edit);
        //}
    ...
    protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            //查看
            if (Convert.ToString(gvRow.Attributes["Status"]) != EnumHelper.GetDescription(MalfunctionHandleStatus.Finished))
            {
                string url = "MalfunctionHandle.aspx?cmd=edit&from=2&&id=" + id;
                //Response.Redirect(url);
                System.Web.UI.ScriptManager.RegisterStartupScript(this, Page.GetType(), "ajaxjs", "window.open('" + url + "','_blank');", true);
            }
            else
            {
                string url = "MalfunctionHandleFinished.aspx?cmd=edit&from=2&&id=" + id;
                //Response.Redirect(url);
                System.Web.UI.ScriptManager.RegisterStartupScript(this, Page.GetType(), "ajaxjs", "window.open('" + url + "','_blank');", true);
            }
        }
        ...
        {
            //查看
            string url = "../MalfunctionReport/ViewMalfunctionSheet.aspx?cmd=view&id=" + id;
            //Response.Redirect(url);
            System.Web.UI.ScriptManager.RegisterStartupScript(this, Page.GetType(), "ajaxjs", "window.open('" + url + "','_blank');", true);
        }
    }
    ...
    protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            e.Row.Attributes["Status"] = EnumHelper.GetDescription(item.Status);
    ...
    private void GetSearchTerm()
    {
        ...
        if (ddlFilterMaintainTeam2.SelectedValue != "0")
        {
            term.MaintainDept = Convert.ToInt32(ddlFilterMaintainTeam2.SelectedValue);
        }
    ...
    private void Filter()
    {
        ...
        term.MaintainDept = Convert.ToInt32(ddlFilterMaintainTeam.SelectedValue); 
        SearchTerm = term;
    }
    /// <summary>
    /// 列表条件筛选
