...
public partial class Module_FM2E_MaintainManager_MalFunctionManager_MalfunctionStationCheck_StationCheckMalfunctionSheet : System.Web.UI.Page
{
    ...
    protected void btCheckPass_Click(object sender, EventArgs e)
    {
        ...
        if (deptBll.GetDepartment(item.MaintainDept).DepartmentType == DepartmentType.MaintainTeamOther)  //外维
        {
            ...
            else
            {
                if (wii.CurrentStateName == "Wait4ElectDeptEngineerConfirm")
                {
                    ...
                    item.Status = MalfunctionHandleStatus.Waiting4MoneyApproval;
                    //malfunctionBll.UpdateWorkflowInstanceNextUserName(item.SheetID);
                }
                else if (wii.CurrentStateName == "Wait4DutyStationConfirm")
                {
                    ...
                    Guid guid = WorkflowHelper.CreateNewInstance(item.SheetID, SGS_MalFunctionWorkflow.WorkflowName);
                    WorkflowHelper.SetStateAndNextUserMachine<SGS_MalFunctionEventService>(guid, WorkflowHelper.GetEventInfoList(SGS_MalFunctionWorkflow.WorkflowName, "New")[0].Name, SGS_MalFunctionWorkflow.WorkflowName, company, out nextUserName);
                    WorkflowHelper.SetStateAndNextUserMachine3<SGS_MalFunctionEventService>(guid, WorkflowHelper.GetEventInfoList(SGS_MalFunctionWorkflow.WorkflowName, "Wait4MaintainDeptManagerConfirm")[0].Name, SGS_MalFunctionWorkflow.WorkflowName, company, nUser, out nextUserName);
                }
                else
                {
                    ...
                    //WorkflowHelper.SetStateAndNextUserMachine1<SGS_MalFunctionEventService>(guid, WorkflowHelper.GetEventInfoList(SGS_MalFunctionWorkflow.WorkflowName, wii.CurrentStateName)[0].Name, SGS_MalFunctionWorkflow.WorkflowName, company, out nextUserName);
                    finishFlag = true;
                }
        ...
        EventMessage.MessageBox(Msg_Type.Info, "操作成功", string.Format("确定故障处理单成功,故障处理单号为：{0}", item.SheetNO), Icon_Type.OK, true, Common.GetHomeBaseUrl("MalfunctionStationCheckList.aspx"), UrlType.Href, "");
    }
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    String company = item.MaintainDeptName;
                    Guid guid = wii.InstanceID;
                    WorkflowHelper.SetStateAndNextUserMachine1<SGS_MalFunctionEventService>(guid, WorkflowHelper.GetEventInfoList(SGS_MalFunctionWorkflow.WorkflowName, wii.CurrentStateName)[1].Name, SGS_MalFunctionWorkflow.WorkflowName, company, out nextUserName);
                    finishFlag = true;
                }
