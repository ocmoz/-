...
public partial class Module_FM2E_Plan_ScheduleActual : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            List<ScheduleInfo> list = bll.GetScheduleActual(planItem.Id);
            ScheduleItems = list;
            FillData();
            if (cmd == "edit")
            {
            ...
            else if (cmd == "view")
            {
                ...
                button.ButtonUrl = string.Format("ScheduleActual.aspx?cmd=edit&id={0}", id);
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            bll.AddAndUpdateScheduleActual(id, item);
        }
        catch (Exception ex)
        {
            EventMessage.MessageBox(Msg_Type.Error, "操作失败", "修改添加失败", ex, Icon_Type.Error, true, "window.history.go(-1)", UrlType.JavaScript, "");
        }
        EventMessage.MessageBox(Msg_Type.Error, "操作成功", "修改添加成功！", Icon_Type.OK, true, Common.GetHomeBaseUrl("ScheduleActual.aspx?cmd=view&id=" + id), UrlType.Href, "");
    }  
}
