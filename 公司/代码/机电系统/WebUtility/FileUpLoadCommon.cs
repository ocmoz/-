...
namespace WebUtility
{
    ...
    public class FileUpLoadCommon
    {
        ...
        private int _width = SystemConfig.Instance.ImageWidth;               // 缩图宽度
        private int _height = SystemConfig.Instance.ImageHeight;               // 缩图高度
        private string _mode = "Auto";        // 缩图模式 "HW"://指定高宽缩放（可能变形）  "W"//指定宽，高按比例 "H"//指定高，宽按比例  "Cut"://指定高宽裁减（不变形） Auto:自动 
        private string _addText = "Water Mark";   // 水印文字
        private string _addPicture = SystemConfig.Instance.UploadPath + "watermark.jpg";   // 水印文字
        ...
        public bool SaveFile(HttpPostedFile fu, bool IsImage, bool IsMakeSize)
        {
            ...
            if (fu.ContentLength > 0)
            {
                ...
                if (fileContentType == "image/x-png" || fileContentType == "image/bmp" || fileContentType == "image/gif" || fileContentType == "image/pjpeg" || fileContentType == "image/jpeg"||fileContentType=="image/jpg")
                {
                ...
                //检测文件扩展名是否正确
                if (!Common.Check_Char_Is(file.Extension.Substring(1).ToLower(), SystemConfig.Instance.UploadFileExt.ToLower()))
                {
                    _errorMsg = string.Format("文件扩展名不符合系统需求:{0}", SystemConfig.Instance.UploadFileExt);
                    //fu.Dispose();
                    return false;
                }
                if (_fileSize / 1024 > SystemConfig.Instance.UploadSizeKb)
                {
                    _errorMsg = string.Format("上传文件超过系统允许大小:{0}K", SystemConfig.Instance.UploadSizeKb);
                    //fu.Dispose();
                    return false;
                }
                ...
                string folderPath = HttpContext.Current.Server.MapPath(_path);
                ...
                if (isfileTypeImages)
                {
                    ...
                    if (!File.Exists(_webFilePath))
                    {
                        try
                        {
                            ...
                            if (_isWatermarkT)
                                AddWaterMarkWord(_webFilePath, _webFilePath_sy);
                            if (_isWatermarkP)
                                AddWaterMarkPic(_webFilePath, _webFilePath_syp, _webFilePath_sypf);
                            if (_isThumbnail)
        ...
        protected void AddWaterMarkWord(string Path, string Path_sy)
        {
