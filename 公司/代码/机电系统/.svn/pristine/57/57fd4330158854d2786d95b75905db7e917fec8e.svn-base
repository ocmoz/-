...
public partial class Module_FM2E_MaintainManager_DailyPatrolManager_DailyPatrolConfig_ViewDailyPatrolConfig : System.Web.UI.Page
{
    ...
    private void ButtonBind()
    {
        if (cmd == "view")
        {
            ...
            button.ButtonUrl = string.Format("EditDailyPatrolConfig.aspx?cmd=edit&id={0}", id);
        ...
        else if (cmd == "delete")
        {
            ...
            {
                EventMessage.MessageBox(Msg_Type.Error, "操作失败", "删除日常巡查计划失败", ex, Icon_Type.Error, true, "window.history.go(-1)", UrlType.JavaScript, "");
            }
            if (bSuccess == true)
            {
                EventMessage.MessageBox(Msg_Type.Info, "操作成功", "删除日常巡查计划(ID:" + id + ")成功！", Icon_Type.OK, true, Common.GetHomeBaseUrl("DailyPatrolConfig.aspx"), UrlType.Href, "");
            }
        }
    }
