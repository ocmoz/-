...
namespace FM2E.Model.Equipment
{
    ...
    /// 不经申请的采购验收回来后的报验记录详情
    /// </summary>
    public class CheckAcceptanceDetailInfo
    {
        public CheckAcceptanceDetailInfo()
		{}
		#region Model
		private long _id;
        ...
        private DateTime _purchasetime = DateTime.MinValue;
        private string _purchaser = "";
		private bool _purchaserconfirm;
        private DateTime _purchaserconfirmtime = DateTime.MinValue;
        ...
        private IList _basebarcode = new List<CheckAcceptanceDetailBarcodeInfo>();
        private IList _childrenbarcode = new List<CheckAcceptanceDetailBarcodeInfo>();
        /// <summary>
        /// 以整体入库的列表
        /// </summary>
        ...
        public IList BarcodeList
        {
            ...
            set
            {
                _barcodelist = value;
                _basebarcode.Clear();
                foreach (CheckAcceptanceDetailBarcodeInfo item in value)
                {
        ...
        public IList BarcodeStrList
        {
            get
            {
                ...
                foreach (CheckAcceptanceDetailBarcodeInfo item in _barcodelist)//_barcodelist已经排序完毕的
                {
                    char key = item.Barcode.Substring(item.Barcode.Length - 2, 1)[0];
                    if (hs.Contains(key))
                    {
