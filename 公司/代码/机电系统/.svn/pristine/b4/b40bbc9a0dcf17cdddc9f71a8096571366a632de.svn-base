...
namespace WebUtility.WebControls
{
    ...
    public class AspNetPager : Panel, INamingContainer, IPostBackEventHandler, IPostBackDataHandler
    {
        ...
        [ReadOnly(false), Browsable(false), ANPDescription("desc_CurrentPageIndex"), ANPCategory("cat_Paging"), DefaultValue(1), DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
        ...
        public int PageSize
        {
            get
            {
                ...
                try
                {
                    _PageSize = SystemConfig.Instance.PageSize;
                    string CookiesKey = string.Format("{0}-PageSize", Common.Get_CookiesName);
                    if (HttpContext.Current.Request.Cookies[CookiesKey] != null)
                    {
                        string pageSize = HttpContext.Current.Request.Cookies[CookiesKey].Value.ToString();
            ...
            set
            {
                ...
                string CookiesKey = string.Format("{0}-PageSize", Common.Get_CookiesName);
                HttpContext.Current.Response.Cookies[CookiesKey].Value = value.ToString();
            }
        }
        ...
        public string CustomInfoHTML
        {
            get
            {
                return string.Format("总记录：<font color='red'>{0}</font>  页码：{1}{3}{2}  每页：", this.RecordCount, this.CurrentPageIndex, this.PageCount,"/");
            }
            set
            {
        ...
        protected override void RenderContents(HtmlTextWriter writer)
        {
            ...
            if (ShowCustomInfoSection == ShowCustomInfoSection.Left)
            {
                ...
                CreateSetPageSizeButton(writer, 10);
                writer.Write("&nbsp;|&nbsp");
                CreateSetPageSizeButton(writer, 20);
                writer.Write("&nbsp;|&nbsp");
                CreateSetPageSizeButton(writer, 50);
                writer.Write("&nbsp;|&nbsp");
                CreateSetPageSizeButton(writer, 100);
                writer.RenderEndTag();
        ...
        /// <summary>
        /// Create set page size button
        /// </summary>
        /// <param name="writer"></param>
        /// <param name="size"></param>
        private void CreateSetPageSizeButton(HtmlTextWriter writer, int size)
        {
            writer.AddAttribute(HtmlTextWriterAttribute.Href, Page.ClientScript.GetPostBackClientHyperlink(this, "PageSize-"+size.ToString()));
        ...
        public void RaisePostBackEvent(string args)
        {
            ...
            int position = CurrentPageIndex * PageSize;
            try
            {
                if (args == null || args == "")
                    args = inputPageIndex;
                int index=args.IndexOf("PageSize-");
                if (index == -1)
                {
                    //换页
                ...
                {
                    //设置每页显示记录数
                    string subStr = args.Substring(9);
                    pageSize = int.Parse(subStr);
                    pageIndex = (int)Math.Ceiling((double)position / pageSize);
                }
            }
            catch { }
            if (cloneFrom != null)
            {
                cloneFrom.PageSize = pageSize;
            }
            else
            {
    ...
    internal sealed class SR
    {
        private SR()
        {
            _rm = new System.Resources.ResourceManager("WebUtility.WebControls.AspNetPager", base.GetType().Assembly);
        }
