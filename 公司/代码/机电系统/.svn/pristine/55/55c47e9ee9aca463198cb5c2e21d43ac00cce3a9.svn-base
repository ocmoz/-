...
public partial class Module_FM2E_MaintainManager_RoutineMaintainManager_RoutineMaintainConfig_ViewRoutineMaintainConfig : System.Web.UI.Page
{
    ...
    private void ButtonBind()
    {
        if (cmd == "view")
        {
            ...
            button.ButtonUrl = string.Format("EditRoutineMaintainConfig.aspx?cmd=edit&id={0}", id);
        ...
        else if (cmd == "delete")
        {
            ...
            {
                EventMessage.MessageBox(Msg_Type.Error, "操作失败", "删除例行保养计划失败", ex, Icon_Type.Error, true, "window.history.go(-1)", UrlType.JavaScript, "");
            }
            if (bSuccess == true)
            {
                EventMessage.MessageBox(Msg_Type.Info, "操作成功", "删除例行保养计划(ID:" + id + ")成功！", Icon_Type.OK, true, Common.GetHomeBaseUrl("RoutineMaintainConfig.aspx"), UrlType.Href, "");
            }
        }
    }
