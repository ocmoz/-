...
public partial class Module_FM2E_MaintainManager_MalFunctionManager_MalfunctionCheck_InteriorEgCheckMalfunctionSheet : System.Web.UI.Page
{
...
*/
    ...
    protected void btCheckPass_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            {
                string title = "您有新的故障处理单要审批，故障处理单号为：" + item.SheetNO;
                string URL = "../MaintainManager/MalFunctionManager/MalfunctionStationCheck/StationMalfunctionCheckList.aspx";
        ...
        EventMessage.MessageBox(Msg_Type.Info, "操作成功", string.Format("验收故障处理单成功,故障处理单号为：{0}", item.SheetNO), Icon_Type.OK, true, Common.GetHomeBaseUrl("InteriorEgMalfunctionCheckList.aspx"), UrlType.Href, "");
    }
    protected void btCheckFailed_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (finishFlag)  //不通过返回修改
            {
                if (otherFlag)
                {
                    string title = "故障处理单验收不通过，请在‘我受理的故障单’中重新进行登记，故障处理单号为：" + item.SheetNO;
                    string URL = "../MaintainManager/MalFunctionManager/MalfunctionMaintain/InteriorMalfunctionSheets.aspx";
        ...
        EventMessage.MessageBox(Msg_Type.Info, "操作成功", string.Format("验收故障处理单成功,该单已返回修复。故障处理单号为：{0}", item.SheetNO), Icon_Type.OK, true, Common.GetHomeBaseUrl("InteriorEgMalfunctionCheckList.aspx"), UrlType.Href, "");
    }
    //********** Modification Finished 2011-11-28 **********************************************************************************************
