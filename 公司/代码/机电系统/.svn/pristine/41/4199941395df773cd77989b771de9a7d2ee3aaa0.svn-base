...
public partial class Module_FM2E_MaintainManager_MalFunctionManager_MalfunctionCheck_CancelMalfunctionSheet : System.Web.UI.Page
{
...
*/
    ...
    protected void btCheckPass_Click(object sender, EventArgs e)
    {
        ...
        MalfunctionHandleInfo item = CurrentSheet;
        item.Status = MalfunctionHandleStatus.Canceled;
        item.CancelApproveName = Common.Get_UserName;
        item.CancelApproveRemark = tbCancelReason.Text.Trim();
        item.CancelApproveTime = DateTime.Now;
        item.CancelApproveResult = "同意撤销";
        item.UpdateTime = DateTime.Now;
        try
            {
                malfunctionBll.UpdateMalfunctionSheetBasicData(item);
                ...
                //        0, null,item.Recorder);
                //**********Modification Finished 2011-6-27**********************************************************************************************
                string title = "您申请的故障处理单号为：" + item.SheetNO+"已同意撤销！";
                string URL = "../MaintainManager/MalFunctionManager/MalfunctionReport/ViewMalfunctionSheet.aspx?cmd=view&id="+item.SheetID;
                string[] receiver = { item.Canceler };
        ...
        EventMessage.MessageBox(Msg_Type.Info, "操作成功", string.Format("撤消故障处理单成功,故障处理单号为：{0}", item.SheetNO), Icon_Type.OK, true, Common.GetHomeBaseUrl("CancelMalfunctionList.aspx"), UrlType.Href, "");
    ...
    protected void btCheckFailed_Click(object sender, EventArgs e)
    {
        ...
        item.CancelApproveRemark = tbCancelReason.Text.Trim();
        item.CancelApproveTime = DateTime.Now;
        item.CancelApproveResult = "不同意撤销";
        item.UpdateTime = DateTime.Now;
        try
        {
            malfunctionBll.UpdateMalfunctionSheetBasicData(item);
            string title = "您申请的故障处理单号为：" + item.SheetNO + "不同意撤销！";
            string URL = "../MaintainManager/MalFunctionManager/MalfunctionReport/ViewMalfunctionSheet.aspx?cmd=view&id=" + item.SheetID;
            string[] receiver = { item.Canceler };
            WorkflowApplication.SendingPendingOrderToUsers(title, Common.Get_UserName, UserData.CurrentUserData.PersonName, URL, 0, null, receiver);
            string title2 = "撤销故障处理单审批不通过，请重新进行登记，故障处理单号为：" + item.SheetNO;
            string URL2 = "../MaintainManager/MalFunctionManager/MalfunctionMaintain/MalfunctionSheets.aspx";
            string[] receiver2 = { item.Receiver };
            WorkflowApplication.SendingPendingOrderToUsers(title2, Common.Get_UserName, UserData.CurrentUserData.PersonName, URL2, 0, null, receiver2);
               
        }
        catch (Exception ex)
        {
            EventMessage.MessageBox(Msg_Type.Error, "操作失败", "审批撤消故障处理单失败", ex, Icon_Type.Error, true, "window.history.go(-1)", UrlType.JavaScript, "");

        }
        EventMessage.MessageBox(Msg_Type.Info, "操作成功", string.Format("审批撤消故障处理单成功,故障处理单号为：{0}", item.SheetNO), Icon_Type.OK, true, Common.GetHomeBaseUrl("CancelMalfunctionList.aspx"), UrlType.Href, "");
