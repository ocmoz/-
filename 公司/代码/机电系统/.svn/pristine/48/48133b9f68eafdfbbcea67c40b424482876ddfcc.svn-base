...
{
    public class PriceEventService : IPriceEventService, IBasicEventService
    {
        ...
        public event EventHandler<ExternalDataEventArgs> PartApproved
        {
            add
            {
                if(_eventList.ContainsValue(value) == false)
                    _eventList.Add("PartApproved", value);
            }
            remove
            {
            }
