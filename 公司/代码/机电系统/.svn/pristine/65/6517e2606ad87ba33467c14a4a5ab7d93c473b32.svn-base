<%@ Page Title="申请记录--当前正在处理" Language="C#" MasterPageFile="~/MasterPage/MasterPage.master"
    AutoEventWireup="true" CodeFile="PurchaseHistory.aspx.cs" Inherits="Module_FM2E_DeviceManager_SpareEquipmentManager_Purchase_PurchaseApply_PurchaseHistory" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="PageBody" runat="Server">
    ...
    <cc1:HeadMenuWebControls ID="HeadMenuWebControls1" runat="server" HeadTitleTxt="采购申请"
        HeadOPTxt="目前操作功能：正在处理的申购单" HeadHelpTxt="点击“申请采购”可以填写新申购单">
        <cc1:HeadMenuButtonItem ButtonIcon="new.gif" ButtonName="申请采购" ButtonUrlType="Href"
            ButtonUrl="PurchaseApply.aspx" />
        <cc1:HeadMenuButtonItem ButtonIcon="select.gif" ButtonName="已经完成的申购单" ButtonUrlType="Href"
            ButtonUrl="PurchaseFinishHistory.aspx" />
        
    </cc1:HeadMenuWebControls>
    <cc2:TabContainer ID="TabContainer1" runat="server" ActiveTabIndex="0" Width="100%">
        <cc2:TabPanel runat="server" HeaderText="正在处理的申购单" ID="TabPanel0">
            <ContentTemplate>
                <div>
                        ...
                        <tr>
                            <td class="Table_searchtitle" colspan="4">
                                <span style="color: Blue">
                                    <%= WebUtility.UserData.CurrentUserData.PersonName %>( <%= WebUtility.UserData.CurrentUserData.UserName %> )</span>正在处理的申购单列表
                            </td>
                        </tr>
                        <tr>
                                        ...
                                        <asp:UpdatePanel ID="UpdatePanel1" runat="server">
                                            <ContentTemplate>
                                                <asp:GridView ID="gridview_PurchaseApplyList" runat="server" AutoGenerateColumns="False"
                                                    ...
                                                    Width="100%" HeaderStyle-HorizontalAlign="center" RowStyle-HorizontalAlign="center" OnRowCommand="OnGridView_RowCommand"
                                                        ...
                                                        <asp:TemplateField HeaderText="修改" ShowHeader="False">
                                                            <ItemTemplate>
                                                                <asp:ImageButton ID="ImageButton_Edit" runat="server" CausesValidation="False" ImageUrl="~/images/ICON/edit.gif"
                                                                    Text="修改" CommandArgument='<%# Eval("ID") %>' CommandName="Edit" Visible='<%# Eval("WorkFlowStateName").ToString() == PurchaseWorkflow.DraftState || Eval("WorkFlowStateName").ToString() == PurchaseWorkflow.ReturnModifyState %>' />
                                                            </ItemTemplate>
                                                            <ItemStyle Width="3%" />
                                                        </asp:TemplateField>
                                                        <asp:TemplateField HeaderText="删除" ShowHeader="False">
                                                            <ItemTemplate>
                                                                <asp:ImageButton ID="ImageButton_Delete" runat="server" CausesValidation="False"
                                                                    Visible='<%# Eval("WorkFlowStateName").ToString() == PurchaseWorkflow.DraftState %>' CommandName="Delete"
                                                                    ImageUrl="~/images/ICON/delete.gif"   CommandArgument='<%# Eval("ID") %>'  Text="删除" OnClientClick="javascript:return confirm('确认删除该项？');" />
                                                            </ItemTemplate>
                                                            <ItemStyle Width="3%" />
                                                        </asp:TemplateField>
                                                        <asp:TemplateField HeaderText="归档" ShowHeader="False">
                                                            <ItemTemplate>
                                                                <asp:ImageButton ID="ImageButton_Archive" runat="server" CausesValidation="False" CommandArgument='<%# Eval("ID") %>'
                                                                    Visible='<%# Eval("WorkFlowStateName").ToString() == PurchaseWorkflow.InWarehouseFinishState || Eval("WorkFlowStateName").ToString() == PurchaseWorkflow.ApprovalFailedState %>' CommandName="Archive"
                                                                    ImageUrl="~/images/ICON/doc.gif" Text="归档" OnClientClick="javascript:return confirm('确认归档该项？');" />
                                                            </ItemTemplate>
                                                            <ItemStyle Width="3%" />
                                                        </asp:TemplateField>
                                                    </Columns>
        ...
        <cc2:TabPanel runat="server" HeaderText="正在处理的申购单查询" ID="TabPanel1">
            <ContentTemplate>
                <table id="Table1" style="width: 100%; border-collapse: collapse; vertical-align: middle;
                    ...
                    <tr>
                        <td class="Table_searchtitle" colspan="4">
                            <span style="color: Blue">
                                    <%= WebUtility.UserData.CurrentUserData.PersonName %>( <%= WebUtility.UserData.CurrentUserData.UserName %> )</span>正在处理的申购单查询
                        </td>
                    </tr>
                    <tr>
