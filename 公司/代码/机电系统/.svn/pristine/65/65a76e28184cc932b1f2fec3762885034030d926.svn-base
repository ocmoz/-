...
return true;
    ...
    function checkGroupForm(fm,causeValidate,group) {

        var grouplenth = 0;
        var groupname = '';
        if (group != undefined) {
            grouplenth = group.length;
            groupname = group;
        }
	...
	//window.onerror = killErrors; 
		for(var i=0;i<fm.length;i++)
		{
			...
			var p = title.lastIndexOf("~" + groupname);
			if(p<0)continue;//忽略title中未定义检查格式的元素
			var info = title.substring(0, p);
			var format = title.substring(p + 1 + grouplenth, title.length);
			var format1=title.substring(p+1+grouplenth,title.length);
			var format2 = title.substring(p + 1 + grouplenth, title.length);
			var name=fm[i].name;
	...
	function checkForm(fm,causeValidate)
	{
	    if(!causeValidate)
	      return true;   //不校验
	...
	function checkSelectOne(sel,group) {
...
**/
function clearForm() {
    var vars = clearForm.arguments;
    if (vars == null)
        return;
        
    for (var i = 0; i < vars.length; i++) {

        var obj = document.getElementById(vars[i]);
        ...
        if (obj.type == "radio") {
            ...
            if (name == "") {
                obj.checked = true;
            }else if (typeof (document.getElementsByName(name).length) == "undefined") {
                //只有一个按钮
                obj.checked = true;
            }
            else {
            ...
            if (name == "") {
                ...
                for (var j = 0; j < checkboxs.length; j++) {
                    if (j == 0) checkboxs[j].checked = true;
                    else checkboxs[j].checked = false;
                }
            }
            ...
            for (var j = 0; j < obj.length; j++) {
                if (j == 0) obj[j].selected = true;
                else obj[j].selected = false;
            }
        } else
            obj.value = "";
    }
...
//检测是否安装了ActiveX控件
function detectPlugin(CLSID, PROPERTY) {
    ...
    try {
        if (eval("objectForDetectPlugin." + PROPERTY) == undefined) {
