...
public partial class Module_FM2E_DeviceManager_SpareEquipmentManager_OutWarehouse_OutWarehouseApply_SelectStorage : System.Web.UI.Page
{
    /// <summary>
    /// 设备管理逻辑处理类
    /// </summary>
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    protected void GridView_StorageList2_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            //鼠标移动到每项时颜色交替效果    
            ...
            {
                if (SelectedValue2.Value.Split(',')[0].Equals(equipmentname))
                    cb.Checked = true;
                else
                    cb.Checked = false;
            }
