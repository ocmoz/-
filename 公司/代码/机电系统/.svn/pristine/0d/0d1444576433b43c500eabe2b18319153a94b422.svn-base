...
public partial class Module_FM2E_PendingOrderMessage_ViewPendingOrder : System.Web.UI.Page
{
    /// <summary>
    /// 消息发送处理业务逻辑处理类
    /// </summary>
    ...
    private void FillData()
    {
        try
        {
            ...
            int listCount = 0;
            IList poList = poBll.GetPendingOrderListByReceiver(pageIndex, AspNetPager1.PageSize, out listCount, Common.Get_UserName);
        ...
        catch (Exception ex)
        {
            ...
            EventMessage.MessageBox(Msg_Type.Error, "操作失败", "读取待办事务失败", ex, Icon_Type.Error, true, "window.history.go(-1)", UrlType.JavaScript, "");
        }
    }
    ...
    protected void gridview_PendingOrderList_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            EventMessage.MessageBox(Msg_Type.Error, "操作失败", "设置待办事务失败", ex, Icon_Type.Error, true, "window.history.go(-1)", UrlType.JavaScript, "");
        }
    }
    ...
    protected void gridview_PendingOrderList_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            var str1 = "上报";
            var str2 = info.Title;
            var str3 = str2.IndexOf(str1);
            if (str3>-1)
            {
                LinkButton lb = e.Row.FindControl("LBtitle") as LinkButton;
                lb.ForeColor = System.Drawing.Color.FromName("Green");
            }
            else
            {
                LinkButton lb = e.Row.FindControl("LBtitle") as LinkButton;
                lb.ForeColor = System.Drawing.Color.FromName("Red");

            }
            
            //           e. Row. Attributes. Add( "OnClick" , String. Format( "javascript:showPopWin('待办事务内容','ViewPendingOrderContent.aspx?id={0}&type={1}&sendfrom={2}&time={3}&attachment={4}', 600, 350, null,true,true);" , info. ID , info. Type , info. SendFrom , info. SendTime. ToString( ) , info. URL ) );
        }
    }
